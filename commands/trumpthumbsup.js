...
class TrumpthumbsupCmd extends Command {
  constructor (client) {
    super(client, {
      name: "trumpthumbsup",
      description: "Makes a thumbs up from trump meme.",
      category: "Meme Maker",
      usage: "",
      enabled: true,
      guildOnly: false,
 ...
 /*   let topText = await this.client.awaitReply(message, "Please tell me what text i should put on top of the meme.", 60000);
    ...
    let bottomText = await this.client.awaitReply(message, "Please tell me what text i should put on bottom of the meme.", 60000);
    if (bottomText === false) {
      reply("Prompt timed out.");
      return msg.delete();
    }
    ...
    if (bottomText.length > 50) return reply("Text on the bottom of the meme must be maximum 50 characters length.");
    ...
    const image = await fsn.readFile("./templates/22.jpg");
  ...
  //  topText = this.client.separateText(topText, 21, 40);
    ...
    await msg.delete();
    message.channel.send("Your thumbs up from trump meme is ready:", attachment);
  }
}

module.exports = TrumpthumbsupCmd;