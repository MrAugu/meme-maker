...
class SmartCmd extends Command {
  constructor (client) {
    super(client, {
      name: "smart",
  ...
  async run (message, args, level, reply) { // eslint-disable-line no-unused-vars
    ...
    let topText = await this.client.awaitReply(message, "Please tell me your smart text.", 60000);
    if (topText === false) {
    ...
    const image = await fsn.readFile("./templates/8.jpg");
    await msg.edit(`${this.client.config.emojis.loading} Priniting your meme... 35%`);
    topText = this.client.separateText(topText, 15, 10);
    ...
    const location = await this.client.textLocation(topText.length, 600, 220, true, true, 0.5, 1);
    await msg.edit(`${this.client.config.emojis.loading} Priniting your meme... 75%`);
    const newMeme = new Canvas(800, 500)
      .addImage(image, 0, 0, 800, 500)
      .setColor("#ffffff")
      .setTextFont('48px Impact')
      .setTextAlign('center')
