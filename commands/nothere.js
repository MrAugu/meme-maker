...
class NothereCmd extends Command {
  constructor (client) {
    super(client, {
      name: "nothere",
      description: "Makes an 'We dont do this here' meme.",
      category: "Meme Maker",
      usage: "",
      enabled: true,
      guildOnly: false,
  ...
  async run (message, args, level, reply) { // eslint-disable-line no-unused-vars
    ...
    let topText = await this.client.awaitReply(message, "Please tell me what we don't do here ", 60000);
    if (topText === false) {
    ...
    const location = await this.client.textLocation(topText.length, 284, 82, true, true, 0.001, 0.5);
    await msg.edit(`${this.client.config.emojis.loading} Priniting your meme... 75%`);
    const newMeme = new Canvas(576, 602)
      .addImage(image, 0, 0, 576, 602)
      .setColor("#000000")
    ...
    await msg.delete();
    message.channel.send("Your 'Outstanding Move' meme is ready:", attachment);
  }
}

module.exports = NothereCmd;
