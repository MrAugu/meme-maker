...
class ScrollCmd extends Command {
  constructor (client) {
    super(client, {
      name: "scrolloftruth",
      description: "Makes a scroll of truth meme.",
      category: "Meme Maker",
      usage: "",
      enabled: true,
      guildOnly: false,
  ...
  async run (message, args, level, reply) { // eslint-disable-line no-unused-vars
    ...
    let topText = await this.client.awaitReply(message, "Please tell me text to write on the scroll of truth.", 60000);
    if (topText === false) {
    ...
    const location = await this.client.textLocation(topText.length, 134, 225, false, true, 0, 0.5);
